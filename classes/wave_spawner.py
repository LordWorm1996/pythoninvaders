...
class WaveSpawner:
    ...
    def spawn_wave_enemies(self, enemy_group, enemy_bullets_group):
        ...
        print(f"Spawned {enemies_spawned} enemies for wave {self.current_wave_data['number']}")
    ...
    def spawn_grid_enemies(self, grid_data, enemy_group, enemy_bullets_group):
        ...
        grid_enemy_dict = {(e.get('row', -1), e.get('col', -1)): e
                          for e in grid_enemies if 'row' in e and 'col' in e}
    ...
    def update(self, dt, enemy_group, enemy_bullets_group=None, attack_pattern=None):
        ...
        if hasattr(self, 'enemies_spawned') and self.enemies_spawned and len(enemy_group) == 0:
            wave_number = self.current_wave_data['number'] if self.current_wave_data else 0
            self.wave_started = False
